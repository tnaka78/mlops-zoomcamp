...
def get_paths(date:str=None):
    ...
    train_date = base_date - datetime.timedelta(days=60)
    valid_date = base_date - datetime.timedelta(days=30)
    train_path = f'./data/fhv_tripdata_{train_date.strftime("%Y-%m")}.parquet'
    valid_path = f'./data/fhv_tripdata_{valid_date.strftime("%Y-%m")}.parquet'
    logger.info(f"Train data path:  {train_path}")
    logger.info(f"Valid data path:  {valid_path}")
